...
# Import game modules
from src.p5.p5_game_state import GameState, StateManager, Portfolio, Deal
from src.p5.p5_simulation_engine import SimulationEngine
from src.p5.p5_scenario_engine import ScenarioEngine
from src.p5.p5_ai_opponent import AIOpponent, RiskProfile
from src.p5.p5_opponent_rules import OpponentRules
from src.p5.p5_rl_training import MockRLAgent
from src.p5.p5_scoring_system import ScoringSystem
from src.p5.p5_game_modes import GameModes, GameMode
from src.p5.p5_dashboard_components import DashboardComponents
from src.core.final_engine import ExportEngine, FinalInfraRiskEngine, demo_payload
...
def page_portfolio_overview():
    ...
    for deal in portfolio.deals.values():
        sector_data[deal.sector] = sector_data.get(deal.sector, 0) + (deal.equity_amount + deal.debt_amount)
...
def page_dscr_trends():
    ...
    for deal in portfolio.deals.values():
        dscr_data.append({
            "deal_name": deal.name,
            "dscr": deal.get_annual_dscr(),
            "dscr_status": "Compliant" if deal.get_annual_dscr() >= 1.25 else "At Risk",
            "sector": deal.sector,
        })
    ...
    if dscr_data:
        ...
        # Bar chart
        fig = px.bar(df, x="deal_name", y="dscr", color="dscr_status",
    ...
    with col2:
        st.metric("Min DSCR", f"{min((d.get_annual_dscr() for d in portfolio.deals.values()), default=0):.2f}x")
    with col3:
        st.metric("Max DSCR", f"{max((d.get_annual_dscr() for d in portfolio.deals.values()), default=0):.2f}x")
    with col4:
...
def page_satellite_viewer():
    ...
    st.info("This is a placeholder for satellite imagery carousel. In production, would show real satellite images of projects.")
    
    # Mock satellite data
    deals = st.session_state.game_state.player_portfolio.deals if st.session_state.game_state else {}
    
    if deals:
        selected_deal = st.selectbox("Select Deal", [d.name for d in deals.values()])
        st.write(f"**Project:** {selected_deal}")
        st.write("📷 Satellite images would be displayed here with timestamps showing project progress")
        st.divider()
        ...
        for i, col in enumerate([col1, col2, col3]):
            with col:
                st.image(f"https://via.placeholder.com/300?text=Image+{i+1}", width='stretch')
...
def page_scoring():
    ...
    for component, details in info["components"].items():
        st.write(f"**{component}:** Max {details['max']} pts (Weight: {details['weight']:.0%})")
...
def page_game_controls():
    ...
    selected_idx = st.selectbox("Choose a game mode", range(len(mode_names)), format_func=lambda i: mode_names[i])
...
def _active_portfolio_payload():
    ...
    if st.session_state.game_state is None or not st.session_state.game_state.player_portfolio.deals:
        return demo_payload()
    ...
    for deal in st.session_state.game_state.player_portfolio.deals.values():
        ...
        payload.append({
            ...
            "tenor_years": deal.tenor_years,
            "probability_of_default": deal.probability_of_default,
            "years_to_maturity": max(1, deal.tenor_years - int(deal.quarters_elapsed / 4)),
        })
    return payload
...
def page_final_engine():
    ...
    st.caption("Dynamic recalculation, covenants, amortization, refinancing, waterfall, scoring, graph propagation, SHAP, exports, and persistence.")
