...
class SiameseLoss(nn.Module):
    ...
    def forward(self, predictions: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        reg_loss = self.regression_loss(predictions['regression'] / 100, targets['regression'] / 100)
        clf_loss = self.classification_loss(predictions['classification'], targets['classification'].long())
        anom_loss = self.anomaly_loss(predictions['anomaly'], targets['anomaly'].float())
        
        total_loss = self.alpha * reg_loss + self.beta * clf_loss + self.gamma * anom_loss
