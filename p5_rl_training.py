...
from p5_game_state import Deal, GameState, Portfolio
...
class RLEnvironment:
    ...
    def get_state_representation(self, portfolio: Portfolio) -> RLState:
        ...
        deployed = sum(
            d.equity_amount + d.debt_amount for d in portfolio.deals.values()
        )
        ...
        sector_hhi = (
            sum((s / total_size) ** 2 for s in sectors.values())
            if total_size > 0
            else 0
        )
        ...
        return RLState(
            ...
            portfolio_leverage=metrics["debt_raised"]
            / (metrics["equity_invested"] + 1),
    ...
    def compute_reward(
        self, portfolio: Portfolio, prev_portfolio: Portfolio = None
    ) -> float:
        ...
        default_losses = (
            portfolio.total_defaults * 50_000_000
        )  # Assume $50M loss per default
