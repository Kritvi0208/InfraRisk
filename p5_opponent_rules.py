...
from p5_game_state import Deal, DealStatus, Portfolio
...
class OpponentRules:
    ...
    def check_portfolio_dscr_minimum(
        self, portfolio: Portfolio, min_dscr: float = 1.25
    ) -> bool:
    ...
    def check_leverage_limit(
        self, portfolio: Portfolio, max_leverage: float = 0.75
    ) -> bool:
    ...
    def check_debt_to_ebitda_limit(
        self, portfolio: Portfolio, max_de: float = 6.0
    ) -> bool:
    ...
    def check_portfolio_default_rate(
        self, portfolio: Portfolio, max_default_rate: float = 0.15
    ) -> bool:
        ...
        if default_rate > max_default_rate:
            self.violations.append(
                RuleViolation(
                    ...
                    severity="critical",
                    affected_deals=[
                        d.deal_id for d in portfolio.deals.values() if d.is_in_default()
                    ],
    ...
    def validate_deal_for_inclusion(
        self, portfolio: Portfolio, deal: Deal
    ) -> Tuple[bool, List[str]]:
