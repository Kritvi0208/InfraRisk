...
def train_satellite_cnn_model(output_dir: Path):
    ...
    with mlflow.start_run(run_name="siamese_cnn"):
        try:
            ...
            logger.info(
                f"✅ Siamese CNN training complete! Best MAPE: {best_mape:.2f}%"
            )
    ...
    with mlflow.start_run(run_name="pinn_physics"):
        try:
            ...
            bridge_opt = torch.optim.Adam(
                bridge_model.parameters(), lr=config["learning_rate"]
            )
            ...
            pavement_opt = torch.optim.Adam(
                pavement_model.parameters(), lr=config["learning_rate"]
            )
            ...
            # Save models
            torch.save(
                bridge_model.state_dict(), output_dir / "models" / "bridge_pinn.pt"
            )
            torch.save(
                pavement_model.state_dict(), output_dir / "models" / "pavement_pinn.pt"
            )
    ...
    with mlflow.start_run(run_name="credit_risk_ensemble"):
        try:
            ...
            sector_labels = np.array(
                [np.random.choice(sectors) for _ in range(len(X_train))]
            )
...
def run_monte_carlo_simulation(ensemble, output_dir: Path):
    ...
    try:
        with mlflow.start_run(run_name="monte_carlo_simulation"):
            ...
            logger.info(
                f"  95% CI: [{pd_stats['ci_95_lower']:.2%}, {pd_stats['ci_95_upper']:.2%}]"
            )
            logger.info(
                f"  Expected Shortfall (CVaR): {pd_stats['expected_shortfall']:.2%}"
            )
            ...
            plt.axvline(
                pd_stats["mean_pd"] * 100,
                color="r",
                linestyle="--",
                label=f"Mean: {pd_stats['mean_pd']:.2%}",
            )
            plt.axvline(
                pd_stats["ci_95_lower"] * 100,
                color="g",
                linestyle="--",
                label=f"95% CI Lower: {pd_stats['ci_95_lower']:.2%}",
            )
            plt.axvline(
                pd_stats["ci_95_upper"] * 100,
                color="g",
                linestyle="--",
                label=f"95% CI Upper: {pd_stats['ci_95_upper']:.2%}",
            )
