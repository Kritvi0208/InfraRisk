...
class AAshtoPSIModel:
    ...
    def compute_psi(
        ...
        """
        traffic_damage = self.traffic_damage_factor(
            traffic_normalized, structural_number, exponent=1.5
        )
        env_factor = self.environmental_degradation(
            temperature, precipitation, freeze_cycles
        )
...
class PINNPavement(nn.Module):
    ...
    def __init__(self, hidden_dims: list = None, physics_weight: float = 1.0):
    ...
    def forward(
        self, x: torch.Tensor, compute_physics: bool = False
    ...
    def layer_by_layer_prediction(
        self, x: torch.Tensor, num_steps: int = 12
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        return {"trajectory": trajectory, "years": torch.arange(num_steps)}
    ...
    def maintenance_threshold_alert(
        self, x: torch.Tensor, threshold: float = 2.5
    ) -> Dict[str, torch.Tensor]:
        """
    ...
    def forward(
        ...
        if physics_residuals is not None:
            physics_loss = self.mse_loss(
                physics_residuals, torch.zeros_like(physics_residuals)
            )
...
if __name__ == "__main__":
    ...
    print(
        f"  PSI range: [{output['prediction'].min():.2f}, {output['prediction'].max():.2f}]"
    )
