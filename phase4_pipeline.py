...
class Phase4Pipeline:
    ...
    def process_contract(
        ...
        try:
            ...
            pdf_structure = self.parser.parse_pdf(
                pdf_path=filename, extract_text=contract_text
            )
            ...
            classification_results = self.classifier.classify_clauses(
            ...
            for classification in classification_results:
                clause = next(
                    (
                        ...
                        if c.clause_id == classification.clause_id
                    ),
                    None,
                )
                if clause:
            ...
            result.confidence_score = min(
                ...
                sum(c.confidence for c in result.extracted_clauses)
                / max(len(result.extracted_clauses), 1),
            )
    ...
    def generate_executive_summary(self, result: ContractAnalysisResult) -> str:
        ...
        if result.extracted_clauses:
            ...
            for clause in sorted_clauses[:3]:
                ...
                lines.append(
                    f"  • {clause.full_reference}: {score:.2f} ({clause.risk_category.value})"
                )
        ...
        if result.comparative_analysis:
            ...
            lines.append("-" * 40)
            lines.append(
                f"Similarity Score: {result.comparative_analysis.similarity_score:.2%}"
            )
            lines.append(f"Deviations: {len(result.comparative_analysis.deviations)}")
            lines.append(
                f"Non-Standard Terms: {len(result.comparative_analysis.non_standard_terms)}"
            )
    ...
    def generate_pipeline_report(self) -> str:
        ...
        if self.results:
            ...
            avg_confidence = sum(r.confidence_score for r in self.results) / len(
                self.results
            )
