...
class MetaLearner(nn.Module):
    ...
    def __init__(
        self, num_base_models: int = 4, num_features: int = 32, hidden_dim: int = 64
    ):
        ...
        self.base_model_weights = nn.Parameter(
            torch.ones(num_base_models) / num_base_models
        )

        # Attention for dynamic weight adjustment
        self.attention_layer = nn.Sequential(
            nn.Linear(num_base_models, num_base_models), nn.Softmax(dim=1)
        )
...
class StackingEnsemble(nn.Module):
    ...
    def __init__(self, num_sectors: int = 3, meta_feature_dim: int = 32):
        ...
        self.meta_learner = MetaLearner(
            num_base_models=4, num_features=meta_feature_dim, hidden_dim=64
        )

        # Sector-specific weightings
        self.sector_weights = nn.Linear(
            16, 4
        )  # Map sector embedding to 4 base model weights
    ...
    def forward(
        ...
        sector_weights = torch.softmax(
            self.sector_weights(sector_embedding), dim=1
        )  # (batch_size, 4)
        ...
        meta_learner_output = self.meta_learner(
            ...
            meta_features=(
                meta_features if meta_features is not None else sector_embedding
            ),
        )
    ...
    def _compute_shap_values(
        self, base_predictions: torch.Tensor, ensemble_prediction: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def blend_predictions(
        ...
        if blend_weights is None:
            blend_weights = {"tft": 0.25, "gnn": 0.25, "pinn": 0.25, "gbt": 0.25}

        batch_size = tft_output.shape[0]

        # Ensure 1D predictions
        tft_flat = (
            tft_output.view(batch_size, -1).mean(dim=1, keepdim=True)
            if tft_output.dim() > 2
            else tft_output
        )
...
class StackingLoss(nn.Module):
    ...
    def forward(
        ...
        if use_l1:
            loss = self.mse_loss(ensemble_pred, targets) + 0.01 * self.l1_loss(
...
if __name__ == "__main__":
    ...
    print(
        f"  Ensemble output range: [{output['ensemble_prediction'].min():.3f}, {output['ensemble_prediction'].max():.3f}]"
    )
