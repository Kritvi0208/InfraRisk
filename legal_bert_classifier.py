...
from contract_types import ClassificationResult, Clause, RiskCategory, SeverityLevel
...
class ClassifierMetrics:
    ...
    per_category_metrics: Dict[RiskCategory, Dict[str, float]] = field(
...
class LegalBertClassifier:
    ...
    def _init_keyword_mappings(self):
        ...
        self.category_keywords = {
            ...
            RiskCategory.OTHER: ["general", "miscellaneous", "additional", "other"],
        }
    ...
    def classify_clause(self, clause: Clause) -> ClassificationResult:
        ...
        top_k_predictions = [
            (cat, min(1.0, score / 10.0)) for cat, score in sorted_scores[:5]
        ]
        ...
        result = ClassificationResult(
            ...
            explanation=self._generate_explanation(
                text_to_classify, predicted_category
            ),
        )
    ...
    def evaluate_predictions(
        ...
        ground_truth: List[Tuple[str, RiskCategory]],
    ) -> ClassifierMetrics:
    ...
    def generate_classification_report(
        ...
        lines.append(
            f"Mean Confidence: {self.get_confidence_distribution(results)['mean']:.4f}\n"
        )
        ...
        for result in sorted_by_conf[:5]:
            lines.append(
                f"  {result.clause_id}: {result.predicted_category.value} ({result.confidence:.4f})"
            )
