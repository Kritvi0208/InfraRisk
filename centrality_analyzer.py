...
class CentralityAnalyzer:
    ...
    def __init__(
        self, adjacency_matrix: Optional[np.ndarray] = None, n_nodes: int = 20
    ):
        """
    ...
    def analyze_portfolio_concentration(self) -> Dict:
        ...
        gini = (
            2
            * np.sum(
                (np.arange(1, self.n_nodes + 1) - (self.n_nodes + 1) / 2)
                * sorted_degree
            )
            / (self.n_nodes * np.sum(sorted_degree))
        )
    ...
    def contagion_risk(self, source_node: int, impact_threshold: float = 0.5) -> Dict:
        ...
        return {
            ...
            "initial_impact": (
                float(self.eigenvector_centrality[source_node])
                if self.eigenvector_centrality is not None
                else 0.0
            ),
            ...
            "risk_level": (
                "HIGH"
                if contagion_ratio > 0.3
                else "MEDIUM" if contagion_ratio > 0.1 else "LOW"
            ),
        }
...
def main():
    ...
    print(
        f"Betweenness centrality range: [{betweenness.min():.3f}, {betweenness.max():.3f}]"
    )

    eigenvector = analyzer.compute_eigenvector_centrality()
    print(
        f"Eigenvector centrality range: [{eigenvector.min():.3f}, {eigenvector.max():.3f}]"
    )
