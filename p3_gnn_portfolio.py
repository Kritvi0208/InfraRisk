...
class RiskPropagationLayer(MessagePassing):
    ...
    def message(
        self, x_j: torch.Tensor, edge_weight: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
...
class CentralityMetrics:
    ...
    def eigenvector_centrality(
        edge_index: torch.Tensor, num_nodes: int, num_iter: int = 10
    ) -> torch.Tensor:
...
class GNNPortfolio(nn.Module):
    ...
    def __init__(
        ...
        self.gnn_layers = nn.ModuleList(
            [RiskPropagationLayer(hidden_dim, hidden_dim) for _ in range(num_layers)]
        )

        # Attention mechanism for edge importance
        self.edge_attention = nn.Sequential(
            nn.Linear(2 * hidden_dim, 64), nn.ReLU(), nn.Linear(64, 1), nn.Sigmoid()
        )
    ...
    def _compute_centrality_metrics(
        ...
        return {
            "betweenness": CentralityMetrics.betweenness_centrality(
                edge_index, num_nodes
            ),
            "eigenvector": CentralityMetrics.eigenvector_centrality(
                edge_index, num_nodes
            ),
    ...
    def cascade_failure_analysis(
        self, node_features: torch.Tensor, edge_index: torch.Tensor, failure_node: int
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        edge_index_reduced = edge_index[
            :, (edge_index[0] != failure_node) & (edge_index[1] != failure_node)
        ]
        ...
        if edge_index_reduced.shape[1] > 0:
            output_reduced = self.forward(
                node_features_reduced, edge_index_reduced, compute_centrality=False
            )
    ...
    def visualize_importance_ranking(
        self, centrality_metrics: Dict[str, torch.Tensor]
    ) -> torch.Tensor:
