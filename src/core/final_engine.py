...
class DebtAmortizationEngine:
    ...
    @classmethod
    def schedule(cls, principal: float, annual_rate: float, years: int) -> List[Dict[str, float]]:
...
class CashWaterfallEngine:
    @staticmethod
    def calculate(deal: ProjectDeal, debt_service: float, tax_rate: float = 0.25, reserve_pct: float = 0.05) -> Dict:
...
class RefinancingRiskEngine:
    @staticmethod
    def assess(deal: ProjectDeal, dscr: float, market_spread_bps: float = 250.0) -> Dict[str, Any]:
        years_to_maturity = deal.years_to_maturity if deal.years_to_maturity is not None else deal.tenor_years
        ...
        score = 100 * (0.30 * maturity_pressure + 0.25 * balloon_pressure + 0.20 * spread_pressure + 0.25 * dscr_pressure)
        return {
...
class PortfolioRulesEngine:
    ...
    def hhi(deals: Iterable[ProjectDeal], field: str) -> float:
        ...
        for deal in deals:
            ...
            total += value
            buckets[getattr(deal, field)] = buckets.get(getattr(deal, field), 0.0) + value
        if total <= 0:
            return 0.0
    ...
    def concentration_details(deals: List[ProjectDeal], field: str) -> Dict[str, Any]:
        ...
        shares = {}
        for deal in deals:
            shares[getattr(deal, field)] = shares.get(getattr(deal, field), 0.0) + deal.portfolio_value
        shares = {key: value / total for key, value in shares.items()} if total > 0 else {}
        return {
    ...
    @staticmethod
    def pd_rejections(deals: List[ProjectDeal], max_pd: float = 0.08) -> List[Dict[str, Any]]:
        return [
            {
...
class SectorScoringEngine:
    ...
    def score(cls, deal: ProjectDeal, dscr: float) -> Dict[str, Any]:
        ...
        return {"score": round(score, 2), "band": "strong" if score >= 75 else "watch" if score >= 55 else "weak"}


class RecommendationEngine:
    @staticmethod
    def recommend(deal_results: List[Dict[str, Any]], portfolio_results: Dict[str, Any]) -> List[str]:
        recs = []
        if portfolio_results["sector_concentration"]["flag"]:
            recs.append("Reduce largest sector exposure; HHI/concentration limit is breached or close to breach.")
        if portfolio_results["country_concentration"]["flag"]:
            recs.append("Add country diversification before sourcing another deal in the largest geography.")
        for result in deal_results:
            if result["decision"] == "reject":
                recs.append(f"Reject {result['name']}: PD is above 8% hard limit.")
            elif result["covenants"]["breached"]:
                recs.append(f"Renegotiate {result['name']}: covenant breach detected before approval.")
            elif result["refinancing_risk"]["band"] == "high":
                recs.append(f"Refinance early or extend tenor for {result['name']}; refinancing risk is high.")
        if not recs:
            recs.append("Portfolio passes hard gates; proceed with monitoring and quarterly recalculation.")
        return recs
        ...
        for i, left in enumerate(deals):
            for j, right in enumerate(deals[i + 1 :], start=i + 1):
                ...
                if same_sector or same_country:
                    ...
                    edges.append({"source": left.deal_id, "target": right.deal_id, "weight": weight})
        return edges

    @staticmethod
    def propagate(deals: List[ProjectDeal], edges: Optional[List[Dict[str, Any]]] = None, iterations: int = 3) -> Dict[str, Any]:
        edges = edges if edges is not None else GraphPropagationEngine.build_edges(deals)
...
class GameCompletionEngine:
    ...
    def score(deals: List[ProjectDeal], portfolio: Dict[str, Any]) -> Dict[str, Any]:
        avg_dscr = np.mean([d["waterfall"]["dscr"] for d in portfolio["deal_results"]]) if deals else 0.0
        default_penalty = 160 * len(portfolio["pd_rejections"])
        concentration_penalty = 80 if portfolio["sector_concentration"]["flag"] else 0
        covenant_penalty = 50 * sum(1 for d in portfolio["deal_results"] if d["covenants"]["breached"])
        sector_score = np.mean([d["sector_score"]["score"] for d in portfolio["deal_results"]]) if deals else 0.0
        total = 400 + min(180, avg_dscr * 75) + sector_score * 3 - default_penalty - concentration_penalty - covenant_penalty
        return {
...
class RLOpponentEngine:
    ...
    def choose_action(portfolio: Dict[str, Any]) -> Dict[str, Any]:
        ...
        high_refi = any(d["refinancing_risk"]["band"] == "high" for d in portfolio["deal_results"])
        if breached or high_refi:
            ...
            action = "hold"
            q_value = 0.55
        return {"action": action, "q_value": q_value, "policy": "deterministic lightweight RL surrogate"}
...
class ContractBenchmarkEngine:
    ...
    def compare(self, sector: str, country: str, project_value: float, tenor_years: int) -> Dict[str, Any]:
        sector_key = sector.lower().replace(" ", "_")
        country_key = country.lower()
        similar = self.db.find_similar_deals(sector_key, country_key, project_value, tenor_years)
        stats = self.db.get_sector_statistics(sector_key)
        return {
            "benchmark_type": "world_bank_ppi_real_transactions",
    ...
    def resolve_nested_clauses(clauses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"resolved": resolved, "dead_links": sorted(set(dead_links)), "circular": circular}
...
class ExportEngine:
    ...
    def to_csv(payload: Dict[str, Any]) -> str:
        ...
        writer = csv.DictWriter(
            output,
            fieldnames=["deal_id", "name", "decision", "dscr", "pd", "sector_score", "refinancing_risk"],
        )
        writer.writeheader()
    ...
    def to_pdf_bytes(payload: Dict[str, Any]) -> bytes:
        ...
        stream = f"BT /F1 11 Tf 50 760 Td ({text[:1800]}) Tj ET".encode("latin-1", errors="replace")
        objects = [
...
class StorageEngine:
    def __init__(self, db_path: Path = DEFAULT_DB_PATH, database_url: Optional[str] = None) -> None:
        ...
        if self.backend == "postgresql":
            with self._connect_postgres() as conn:
                conn.execute(
                    ...
                    """,
                    (run_id, datetime.now(timezone.utc).isoformat(), json.dumps(payload, default=str)),
                )
            return
    ...
    def latest_runs(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        if self.backend == "postgresql":
            ...
            return [{"run_id": r[0], "created_at": r[1], "payload": json.loads(r[2])} for r in rows]
...
class FinalInfraRiskEngine:
    ...
    def recalculate_portfolio(self, raw_deals: List[Dict[str, Any]], persist: bool = True) -> Dict[str, Any]:
        deals = [ProjectDeal.from_dict(d) for d in raw_deals]
        deal_results = []
        for deal in deals:
            schedule = DebtAmortizationEngine.schedule(deal.debt_amount, deal.coupon_rate, deal.tenor_years)
            ...
            decision = "reject" if deal.probability_of_default > 0.08 else "review" if covenants["breached"] else "approve"
            deal_results.append(
                {
                    "deal_id": deal.deal_id,
                    "name": deal.name,
        ...
        portfolio = {
            ...
            "sector_concentration": PortfolioRulesEngine.concentration_details(deals, "sector"),
            "country_concentration": PortfolioRulesEngine.concentration_details(deals, "country"),
        ...
        portfolio["recommendations"] = RecommendationEngine.recommend(deal_results, portfolio)
        ...
        portfolio["run_id"] = f"run_{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S%f')}"
        if persist:
    ...
    def _shap(self, portfolio: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "local": [interpreter.local_feature_impact(i, shap_values) for i in range(len(rows))],
        }
...
DATA_PROVENANCE = {
    "real_data": [
        ...
        "data/processed/real_data_availability_report.json - strict availability report; flags old generated fallbacks",
    ...
    "model_outputs_not_source_data": [
        "dashboard_v2_complete.py - stress tests and forecasts are model outputs computed from real PPI/WDI/CDS portfolio inputs",
        "benchmark_database.py - comparable transactions are loaded from real World Bank PPI rows",
        "shap_interpreter.py - SHAP-inspired local explanation approximation over computed model features",
        "Graph propagation in final_engine.py - deterministic lightweight GNN-style message passing over portfolio inputs",
        "api_server.py /portfolio/demo - demo endpoint only; production dashboard builds from real local files",
    ],
}
