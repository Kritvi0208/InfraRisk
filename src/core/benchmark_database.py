...
class BenchmarkDatabase:
    ...
    def _load_ppi_transactions(self) -> List[BenchmarkTransaction]:
        ...
        equity = (
            pd.to_numeric(df.get("equity", 0), errors="coerce").fillna(0.0) * 1_000_000
        )
        ...
        for idx, row in df.iterrows():
            ...
            sector = (
                _decode_code(row.get("sector"), PPI_SECTOR_CODES, "sector")
                .lower()
                .replace(" ", "_")
            )
            country = _decode_code(
                row.get("country"), PPI_COUNTRY_CODES, "country"
            ).lower()
            risk_score = {
                "operational": 2.0,
                "concluded": 2.2,
    ...
    def find_similar_deals(
        ...
        candidates.sort(key=lambda x: abs(x.project_value - project_value))
    ...
    def _get_top_sectors_for_country(
        self, country: str, top_n: int = 3
    ) -> List[Tuple[str, int]]:
    ...
    def get_statistical_comparison(
        ...
        comparison = {
            "project_value_deviation": (
                target_txn.project_value - mean(comparable_values)
            )
            / mean(comparable_values),
            "tenor_deviation": (target_txn.tenor_years - mean(comparable_tenors))
            / mean(comparable_tenors),
            "equity_deviation": (
                target_txn.equity_percentage - mean(comparable_equities)
            )
            / mean(comparable_equities),
            "risk_deviation": (target_txn.risk_score - mean(comparable_risks))
            / mean(comparable_risks),
    ...
    def detect_outliers(
        self, sector: str, country: Optional[str] = None, std_threshold: float = 2.0
    ) -> List[Dict]:
        ...
        for txn in txns:
            z_score = (
                abs((txn.project_value - mean_val) / std_val) if std_val > 0 else 0
            )
        ...
        for txn in txns:
            z_score = (
                abs((txn.risk_score - mean_risk) / std_risk) if std_risk > 0 else 0
            )
            if z_score > std_threshold:
                outliers.append(
                    {
