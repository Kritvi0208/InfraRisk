...
from contract_types import (
    Clause, ComparativeAnalysisResult, BenchmarkTransaction,
    SeverityLevel
)
...
class ComparativeAnalyzer:
    ...
    def analyze_contract(
        ...
        similarity = self._calculate_similarity(
            project_value, project_value_benchmark=median_comparable.project_value,
            tenor=tenor_years, tenor_benchmark=median_comparable.tenor_years,
            equity=equity_percentage, equity_benchmark=median_comparable.equity_percentage,
        )
    ...
    def _calculate_similarity(
        ...
        value_diff = min(1.0, abs(project_value - project_value_benchmark) / max(project_value, project_value_benchmark, 1))
        tenor_diff = min(1.0, abs(tenor - tenor_benchmark) / max(tenor, tenor_benchmark, 1))
    ...
    def _detect_deviations(
        ...
        if abs(equity_percentage - bench_equity) > 5.0:
            ...
            deviations.append({
                ...
                'severity': SeverityLevel.MEDIUM if abs(equity_percentage - bench_equity) > 10 else SeverityLevel.LOW,
            })
        ...
        if abs(milestone_count - bench_milestones) > 2:
            ...
            deviations.append({
                ...
                'severity': SeverityLevel.LOW if abs(milestone_count - bench_milestones) <= 3 else SeverityLevel.MEDIUM,
            })
        
        # Check financial covenant stringency
        clause_refs = [c.full_reference.lower() for c in clauses]
        has_covenants = any('covenant' in ref or 'dscr' in ref or 'leverage' in ref for ref in clause_refs)
    ...
    def _identify_non_standard_terms(
        ...
        if 'termination for convenience' in clause_text_lower:
            non_standard.append("Unusual: Termination for Convenience (not standard in comparable deals)")
        ...
        if 'sponsor support' not in clause_text_lower and benchmark_txn.equity_percentage > 25:
    ...
    def _assess_deviation_severity(self, deviations: List[Dict]) -> SeverityLevel:
        ...
        max_severity = max(
            (severity_values.get(d.get('severity', SeverityLevel.LOW), 2) for d in deviations),
            default=1
        )
    ...
    def _detect_outlier_conditions(
        ...
        if abs(project_value - avg_value) > 2 * (sum((v - avg_value) ** 2 for v in values) / len(values)) ** 0.5:
    ...
    def generate_comparative_report(
        ...
        lines.append(f"Benchmark Transaction: {analysis.benchmark_transaction.transaction_id}")
