...
from contract_types import NamedEntity, EntityType
...
class ContractNER:
    ...
    def extract_entities(self, text: str) -> List[NamedEntity]:
        ...
        # Extract sponsors
        entities.extend(self._extract_with_pattern(
            text, self.sponsor_patterns, EntityType.SPONSOR
        ))
        
        # Extract lenders
        entities.extend(self._extract_with_pattern(
            text, self.lender_patterns, EntityType.LENDER
        ))
        ...
        # Extract dates
        entities.extend(self._extract_with_pattern(
            text, self.date_patterns, EntityType.DATE
        ))
        ...
        # Extract parties
        entities.extend(self._extract_with_pattern(
            text, self.party_patterns, EntityType.PARTY
        ))
    ...
    def _extract_amounts(self, text: str) -> List[NamedEntity]:
        ...
        for pattern in self.amount_patterns:
            ...
            for match in matches:
                ...
                currency = match.group(1) if match.groups() and match.group(1) else "USD"
                amount = match.group(2) if match.groups() and len(match.groups()) > 1 else match.group(0)
    ...
    def extract_locations(self, text: str) -> List[NamedEntity]:
        ...
        locations = [
            'United States', 'USA', 'India', 'China', 'Brazil', 'Mexico',
            'United Kingdom', 'UK', 'Germany', 'France', 'Singapore',
            'Australia', 'Canada', 'UAE', 'Saudi Arabia', 'New York', 'California'
        ]
        
        for location in locations:
            matches = re.finditer(r'\b' + re.escape(location) + r'\b', text, re.IGNORECASE)
            for match in matches:
                entity = NamedEntity(
                    text=match.group(0),
    ...
    def generate_training_data(self, num_samples: int = 100) -> List[Dict]:
        ...
        sample_sponsors = ['PowerCorp Ltd', 'Infrastructure Partners', 'Green Energy Solutions']
        sample_lenders = ['International Finance Bank', 'Development Finance Institution']
        sample_amounts = ['500', '750', '1000']
        sample_dates = ['January 15, 2024', 'March 20, 2024']
        sample_milestones = ['Financial Close', 'Construction Start', 'Commercial Operations']
        sample_covenants = ['Debt Service Coverage Ratio of 1.2x', 'Minimum Liquidity of $50M']
    ...
    def evaluate(self, 
                 predicted: List[NamedEntity], 
                 ground_truth: List[NamedEntity]) -> NERMetrics:
