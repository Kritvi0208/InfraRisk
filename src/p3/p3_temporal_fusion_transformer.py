...
class TemporalFusionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.layer_norms_attn = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(num_layers)])
        self.layer_norms_ff = nn.ModuleList([nn.LayerNorm(d_model) for _ in range(num_layers)])
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        self.attention_weights_history = torch.stack(attention_weights_list, dim=0).mean(dim=0)
        ...
        for head in self.quantile_heads:
            quantile_preds = head(x_global)  # (batch_size, len(horizons) * num_quantiles)
            quantile_preds = quantile_preds.view(batch_size, len(self.horizons), self.num_quantiles)
...
if __name__ == "__main__":
    ...
    assert output['quantiles'].shape == (8, 3, 3), f"Expected (8, 3, 3), got {output['quantiles'].shape}"
    ...
    print(f"  Quantiles shape: {output['quantiles'].shape} (batch, horizons, quantiles)")
