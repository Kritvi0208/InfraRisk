...
def download_world_bank_wdi():
    ...
    if all_data:
        ...
        print(
            f"   ✅ Saved {len(df_wide)} country-year records to data/raw/worldbank/wdi_macro.csv"
        )
        return df_wide
    else:
...
def download_world_bank_ppi():
    ...
    ppi_url = (
        "https://ppi.worldbank.org/content/dam/PPI/documents/PPI_2023_H1_Snapshot.xlsx"
    )
        ...
        bridges.append(
            {
                ...
                "deck_condition": round(
                    np.clip(condition + np.random.normal(0, 0.5), 1, 9), 1
                ),
                "superstructure_condition": round(
                    np.clip(condition + np.random.normal(0, 0.5), 1, 9), 1
                ),
                "substructure_condition": round(
                    np.clip(condition + np.random.normal(0, 0.5), 1, 9), 1
                ),
                ...
                "sufficiency_rating": round(
                    np.clip(100 - age * 0.8 + np.random.normal(0, 10), 0, 100), 1
                ),
                "structurally_deficient": condition < 5,
                "remaining_useful_life_years": max(
                    0, int(75 - age + np.random.normal(0, 5))
                ),
            }
        )
        ...
        transactions.append(
            {
                ...
                "debt_value_usd_million": round(
                    capex * np.random.uniform(0.6, 0.85), 1
                ),
                "sector": np.random.choice(
                    ["Roads", "Power", "Ports", "Airports", "Water", "Telecom"]
                ),
            ...
            rows.append(
                {
                    ...
                    "sovereign_bond_yield": round(
                        spread / 100 + 3 + np.random.normal(0, 0.3), 2
                    ),
                }
            )
