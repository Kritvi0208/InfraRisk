...
class MonteCarloPDEngine:
    ...
    def calculate_portfolio_pd(
        ...
        portfolio_pds = (
            base_pd
            + (ir_shocks * ir_sensitivity)
            + (default_shocks * default_sensitivity)
        )
    ...
    def calculate_losses(
        self, defaults: np.ndarray, lgd: float = 0.4, ead_mean: float = 100000
    ) -> np.ndarray:
        """
        ...
        eads = np.random.lognormal(
            np.log(ead_mean), 0.3, (self.n_scenarios, self.n_assets)
        )
    ...
    def run_simulation(self, verbose: bool = True) -> Dict:
        ...
        portfolio_pds, ir_shocks, default_shocks, delay_shocks = (
            self.calculate_portfolio_pd()
        )
        ...
        if verbose:
            ...
            print(
                f"  Mean: {self.confidence_intervals['mean']:.3f}% ± {self.confidence_intervals['std']:.3f}%"
            )
