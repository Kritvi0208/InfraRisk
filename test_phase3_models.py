...
def test_siamese_cnn():
    ...
    assert outputs["regression"].shape == (
        4,
        1,
    ), f"Regression shape mismatch: {outputs['regression'].shape}"
    assert outputs["classification"].shape == (
        4,
        5,
    ), f"Classification shape mismatch: {outputs['classification'].shape}"
    assert outputs["anomaly"].shape == (
        4,
        1,
    ), f"Anomaly shape mismatch: {outputs['anomaly'].shape}"
    assert outputs["features"].shape == (
        4,
        2048,
    ), f"Features shape mismatch: {outputs['features'].shape}"
...
def test_temporal_fusion_transformer():
    ...
    assert output["quantiles"].shape == (
        ...
        3,
        3,
    ), f"Quantiles shape mismatch: {output['quantiles'].shape}"
    assert output["attention_weights"] is not None, "Attention weights are None"
    assert output["temporal_features"].shape == (
        8,
        12,
        256,
    ), f"Temporal features shape mismatch"
...
def test_pinn_fatigue():
    ...
    from p3_pinn_fatigue import FatigueLoss, PINNFatigue
    ...
    assert output["physics_residual"].shape == (
        4,
        1,
    ), f"Physics residual shape mismatch"
...
def test_pinn_pavement():
    ...
    from p3_pinn_pavement import PavementLoss, PINNPavement
    ...
    assert torch.all(output["prediction"] >= 1.5) and torch.all(
        output["prediction"] <= 4.5
    ), "PSI out of range"
    ...
    assert alert["needs_maintenance"].shape == (
        4,
        1,
    ), f"Maintenance alert shape mismatch"
...
def test_gnn_portfolio():
    ...
    assert output["risk_scores"].shape == (
        num_projects,
        1,
    ), f"Risk scores shape mismatch"
    ...
    assert cascade["original_risk"].shape == (
        num_projects,
        1,
    ), f"Cascade original risk shape mismatch"
...
def test_gradient_boosting():
    ...
    from p3_gradient_boosting import BayesianOptimizer, CreditRiskLoss, XGBLGBEnsemble
    ...
    assert output["feature_importance"].shape == (
        50,
...
def test_ensemble_stacking():
    ...
    assert output["ensemble_prediction"].shape == (
        batch_size,
        1,
    ), f"Ensemble prediction shape mismatch"
    assert output["base_predictions"].shape == (
        batch_size,
        4,
    ), f"Base predictions shape mismatch"
    assert output["shap_values"].shape == (batch_size, 4), f"SHAP values shape mismatch"

    blended = model.blend_predictions(
        tft_output, gnn_output, pinn_output, gbt_output, sector_ids
    )
