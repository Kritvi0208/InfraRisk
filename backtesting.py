...
class BacktestingFramework:
    ...
    def compute_psi(
        self, y_pred_dev: np.ndarray, y_pred_test: np.ndarray, n_bins: int = 10
    ) -> float:
        """
    ...
    def backtest_historical(
        ...
        for period in range(n_periods):
            ...
            y_pred = np.clip(np.random.beta(2, 50, n_samples) + drift, 0, 1)
    ...
    def run_full_backtest(self, verbose: bool = True) -> Dict:
        ...
        if verbose:
            ...
            print(f"\nModel Stability:")
            print(
                f"  PSI: {psi:.4f} ({'Acceptable' if psi < 0.1 else 'Warning' if psi < 0.25 else 'Degradation'})"
            )
            print(f"\nCalibration:")
