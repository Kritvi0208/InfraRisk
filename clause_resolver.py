...
class ClauseResolver:
    ...
    def generate_report(self) -> str:
        ...
        lines.append(f"Total Dependencies: {sum(len(v) for v in self.graph.edges.values())}")
        ...
        sorted_by_depth = sorted(self.graph.depth_map.items(), key=lambda x: x[1], reverse=True)
